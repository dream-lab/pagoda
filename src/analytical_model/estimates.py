...
import torch
import torch.nn as nn  # Import the custom module
import onnx
...
class LayerInfoCollectorONNX:
    ...
    def compute_flops_mem_onnx(self, node, input_shape, output_shape):
        ...
        if node.op_type == "Conv":
            ...
            input_shapes = [self._get_tensor_shape(inp) for inp in node.input]
            # input_shapes[0][1] = 8  # temp for lstm
            ...
            # output_shape[0] = 8  # temp for lstm
            ...
            num_gates = 4  # LSTM typically has 4 gates (input, forget, output, and candidate)
            
            # Input dimensions
            batch_size = input_shape[0]
            sequence_length = input_shape[1]  # Comes from the input data not a model property
            sequence_length = 8
            ...
            # FLOPs computation
            # Input-to-hidden matrix multiplication: 4 * input_dim * hidden_size
            input_to_hidden_flops = num_gates * input_dim * hidden_size * batch_size * sequence_length
            
            # Hidden-to-hidden matrix multiplication: 4 * hidden_size * hidden_size
            hidden_to_hidden_flops = num_gates * hidden_size * hidden_size * batch_size * sequence_length
            
            # Additions for gates: 4 * hidden_size
            gate_additions = num_gates * hidden_size * batch_size * sequence_length
            
            # Activations (sigmoid and tanh): 4 (3 sigmoids + 1 tanh) * hidden_size
            activation_flops = 3 * hidden_size * batch_size * sequence_length * 4 # Sigmoid
            activation_flops += hidden_size * batch_size * sequence_length * 5   # Tanh
            
            # Element-wise multiplications: 3 * hidden_size
            elementwise_flops = 3 * hidden_size * batch_size * sequence_length
            
            # Total FLOPs
            flops = input_to_hidden_flops + hidden_to_hidden_flops + gate_additions + activation_flops + elementwise_flops
            print('[I2H]:', input_to_hidden_flops / flops)
            print('[H2H]:', hidden_to_hidden_flops / flops)
            print('[GA]:', gate_additions / flops)
            print('[Act]:', activation_flops / flops)
            print('[Elem]:', elementwise_flops / flops)
            ...
            hidden_cell_memory = 2 * batch_size * hidden_size * dtype_size  # Hidden + Cell states
